...
public class AsientoContableDAOImpl implements AsientoContableDAO {
    ...
    public List<ExcelAsientoContableDTO> busquedaExcel(String periodo, Integer id) {
        try(var session = sessionFactory.openSession()) {
            ...
            session.doWork(cnx -> {
                ...
                int index = 0;
                ps.setObject(++index, periodo, Types.VARBINARY);
                ps.setObject(++index, id, Types.INTEGER);
                ResultSet rs = ps.executeQuery();
                if (Objects.nonNull(rs)){
                    while (rs.next()){
                        ExcelAsientoContableDTO oMapper = mapperResultsetRecord(rs);
                        lista.add(oMapper);
                    }
                    rs.close();
                }
    ...
    private ExcelAsientoContableDTO mapperResultsetRecord(ResultSet rs) throws SQLException {
